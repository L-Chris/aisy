...
export const createQueue = (params: {
    ...
    const onSuccess = (e: any) => {
        if (params.showProgress) {
            ...
            console.log(`[${params.name}] Progress ${results.length}/${(q.length || (q.length - 1)) + results.length}`)
        }
    }
        ...
        start() {
            return new Promise((resolve, reject) => {
                q.start(err => {
                    ...
                    console.log(`
[${params.name}] Finished ${results.length - failedJobs.length}/${results.length}
...
export const getErrorMessage = (err: any) => {
    if (err?.response?.data) {
      if (typeof err?.response?.data === 'string' && err?.response?.data?.length >= 1000) return err?.response?.data.slice(100)
